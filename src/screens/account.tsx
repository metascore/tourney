...
export default function Account () {
    ...
    const { actor, isConnected : isConnectedS, principal : principalS, disconnect : disconnectS } = useStoic();
    ...
    return (
        <div style={{display: 'flex', flexDirection: 'column', gap: '20px', alignItems: 'center'}}>
            <Panel>
                <h1>Account</h1>
                <div>Stoic: {principalS?.toText()}</div>
                <div>Plug: {principalP?.toText()}</div>
